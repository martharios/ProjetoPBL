...
class MensagemController {
    ...
    def index() {
        ...
        withSoap(serviceURL: 'http://localhost:8080/SISAP/services/SISAPMail?wsdl') {
            def response = send {
                body {
                    sendMail {
                        titulo("mensagem")
                        mensagem("mensagem")
                        destinatario("philliparente@gmail.com")
                    }
                }
            }
            def result = response.testeResult
...
//        redirect(url: '/services/SISAPMail?wsdl')
